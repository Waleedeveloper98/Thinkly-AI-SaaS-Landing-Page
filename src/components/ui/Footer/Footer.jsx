...
const Footer = () => {
  return (
    <Container>
      <footer className="pt-10 px-4 md:px-12 lg:px-24 pb-8 bg-[var(--bg-main)]">
        <div className="top flex flex-col gap-10 pb-10 md:flex-row md:justify-between">
          <div className="flex flex-col gap-2">
            <h4 className="logo font-semibold text-xl text-[var(--text-heading)] md:text-3xl">
              Thinkly
            </h4>
            ...
            <div className="products-links flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                {footerLinks.slice(0, 3).map((link, i) => (
                  <a
                    className="text-[var(--text-muted)] transition-all duration-300 hover:text-[var(--text-body)] lg:text-sm"
                    key={i}
                    href={link.href}
                  >
                    {link.text}
